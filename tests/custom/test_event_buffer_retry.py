...
class TestEventBufferRetry(unittest.TestCase):
    ...
    def test_flush_with_retry(self):
        ...
        self.mock_sender.send_batch.side_effect = [
        ...
        with patch("time.sleep") as mock_sleep:  # Mock sleep to speed up test
            ...
            self.assertEqual(self.mock_sender.send_batch.call_count, 3)
    ...
    def test_exponential_backoff_timing(self):
        ...
        try:
            event = MagicMock(spec=CreateEventRequestBody)
            buffer.events = [event]
            mock_sender.send_batch.side_effect = Exception("always fails")
    ...
    def test_flush_with_max_retries_exhausted(self):
        ...
        self.mock_sender.send_batch.side_effect = Exception("API failure")

        with patch("time.sleep") as mock_sleep:  # Mock sleep to speed up test
            self.event_buffer._flush()

            # Verify all retry attempts were made
            self.assertEqual(self.mock_sender.send_batch.call_count, DEFAULT_MAX_RETRIES + 1)
...
class TestAsyncEventBufferRetry:
    ...
    async def buffer_with_mock_periodic_flush(self):
        ...
        with patch.object(buffer, '_periodic_flush', dummy_periodic_flush):
            ...
            yield buffer, mock_sender, mock_logger
    ...
    async def test_flush_with_retry(self, buffer_with_mock_periodic_flush):
        ...
        buffer, mock_sender, mock_logger = buffer_with_mock_periodic_flush
        ...
        # Configure mock to fail twice then succeed on third attempt
        mock_sender.send_batch.side_effect = [
            Exception("API failure 1"),
            Exception("API failure 2"),
            None  # Success
        ]
    ...
    async def test_flush_with_max_retries_exhausted(self, buffer_with_mock_periodic_flush):
        ...
        # Configure mock to always fail
        mock_sender.send_batch.side_effect = Exception("API failure")

        with patch("asyncio.sleep", return_value=None) as mock_sleep:
            await buffer._flush()

            # Verify all retry attempts were made
            assert mock_sender.send_batch.call_count == DEFAULT_MAX_RETRIES + 1
